...
export const WidenAsset = ({types, id, width, sourceID}) => {
    ...
    if (loading) {
        return <p>{t('loading.bgImage')}</p>;
    }// <img src={`https://via.placeholder.com/${width}x768/09f/fff?text=Loading`} alt="loading"/>;
    ...
    switch (true) {
        case types.includes(cndTypes.WIDEN_IMAGE) && Boolean(imageURL):
            return <WidenImage imageURL={imageURL} title={title} width={width}/>;

        case types.includes(cndTypes.WIDEN_VIDEO) && Boolean(videoURL):
